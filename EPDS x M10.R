...
dados <- read_excel("C:/Users/julin/Documents/FACULDADE/7 período/TCC/Análises em R/Tabelas dos dados/análises específicas (sono X depressão)/M10 x EPDS.xlsx")

# 3. Verificar os nomes das colunas
print(colnames(dados))

# 4. Renomeando colunas, se necessário 
colnames(dados) <- c("M10", "EPDS")
...
grupo_sem_depressao <- dados$M10[dados$EPDS < 12]
grupo_com_depressao <- dados$M10[dados$EPDS >= 12]
...
boxplot(M10 ~ (EPDS >= 12), data = dados,
        names = c("Sem Depressão", "Com Depressão"),
        main = "Distribuição de M10 por Grupo",
